...
- **Runs automatically on page load** — checking every time the app opens suits daily use and catches moves sooner than a weekly timer would. No need to click anything; the summary panel below the table populates itself.
- **Refresh all prices**: fetches a live price for every holding at once, updates each holding's Current price, and checks each one for two things — a ≥10% breakout above its trailing high, or price at/below its stop loss.
- **Where prices come from**: a holding with an **ISIN** is priced in EUR directly from **Lang & Schwarz**, the exchange Trade Republic routes its orders through — so it matches what the broker shows, typically within about 0,2%. Such holdings are marked **(LS)** next to the price. A holding without an ISIN falls back to a Finnhub USD quote converted at the live ECB USD/EUR rate, moved forward by each day's percentage change rather than overwritten, since Finnhub's absolute price never exactly matches the broker's. Prices aren't fetched at weekends, when no exchange is open.
- Per-row **Refresh** button does the same for a single holding.
- **Manual-price holdings**: some instruments on Trade Republic have no matching free price feed — the common case is a US ADR that bundles several of the underlying ordinary shares, so the quote comes back a whole multiple away from the price your broker shows. Refresh **detects** this (a fetched price more than 10× off your saved one) and, instead of overwriting, asks whether to keep your own price and mark the holding **manual**. A manual holding shows "manual price" instead of a Refresh button and keeps the price you type in the Edit form; the fetched price itself is never used, though Refresh-all still moves it by the day's percentage change when the ticker can be fetched. You can toggle it back on anytime via the "manual price" checkbox in Edit. **Adding the ISIN is the better fix**: Lang & Schwarz prices the real underlying share, so the mismatch disappears and manual mode isn't needed.
...
- **Stop-loss hit alert**: shows your pre-set exit plan, estimated gain/loss vs. buy-in, what selling would actually add to this year's tax (26,375% Abgeltungssteuer, after netting against the losses you've already realised this year — so a sale covered by earlier losses shows no extra tax), and the latest analyst consensus (Finnhub) — facts to decide fast, not a directive recommendation.
...
- All money values and percentages display in EU format (`1.234,56`, not `1234.56`).
...
- **Sale price per share isn't a separate input** — it's derived as `total sum ÷ shares sold` and stored alongside the total, since typing both independently risks the two disagreeing by a few cents. **Total spend** — what the sold shares originally cost, taken from the oldest lots first (FIFO) — is stored too, alongside Total received, so both sides of the trade are visible side by side in History. Realized gain/loss (`total received − total spend`) is computed the same way.
...
- If shares sold equals the full holding, the row is removed entirely (full liquidation). If it's less, the row stays with its share count reduced: the shares come off the oldest lots first, so the lots left behind keep their own buy-in prices and dates, while stop loss, trailing high and exit plan stay as they were.
- Every sell writes one entry to the **History** tab (`data/sales_history.json`): ticker, shares sold, sale price, total received, cost basis, realized gain/loss, sell date/time. **Tax is shown for the year, not per sale**: a summary above the table adds up the profits and the losses, nets them the way German capital-gains tax does (losses on shares offset gains on shares), applies the €1.000 Sparerpauschbetrag to a positive total, and says whether anything is owed or a loss carries forward. Entries are sortable-by-date (newest first) and each has its own **Remove** button — there's no separate "delete a holding by mistake" action; if you added a holding in error, Sell it (any values) and then Remove that entry from History, since a data-entry mistake isn't a real transaction worth keeping a permanent record of.
...
- This only covers screening/opportunity discovery, not your actual holdings' ranks unless they happen to appear in that screen — in practice most of a real portfolio won't be on a Rank 1 list. Zacks offers a separate "My Portfolio" email alert feature that could cover your own positions — worth subscribing to as a complementary, official-channel alternative to another CSV.
...
- **No ticker validation.** Whatever you type is stored as-is (uppercased). A mistyped ticker isn't caught at entry — it surfaces later as a fetch error on that row the first time you Refresh it, which is how a real typo was found once.
- **Holdings without an ISIN are priced approximately.** They fall back to a Finnhub USD quote converted to EUR — very close, but not the broker's exact figure. Adding the ISIN fixes it.
- **The tax figure is an estimate, not a filing.** It only sees sales recorded in this tool, so an allowance or losses used in another account aren't known to it; church tax is left out; and it assumes every sale is a share, since losses on shares can only offset gains on shares.
...
Built for someone moving from earning to living off their investments. That changes the questions: not only what to buy, but when to sell, how much to take out, and what the tax costs along the way.

The brokers used don't offer trailing stops, which lock in gains by moving a stop-loss up as a price rises. The tool tracks them by hand and says when to raise one at the broker. The rest grew from real decisions: what a stock's price and profits are doing, which holdings move together, what a sale really costs in German tax once losses are netted off, and keeping a written reason for every position.

Two rules shape all of it. It runs locally, so the data stays on one machine. And it never tells you what to do. It shows the facts and leaves the decision to you.
